...
**Follow-up investigation.** We tested one specific hypothesis for the batch 64/256 regression above — that it's caused by kernel-dispatch overhead — by building a custom fused CPU kernel and re-running this exact comparison as a three-way benchmark (dense / TensorFold-torch / TensorFold-fused). The hypothesis was falsified: the fused kernel performs within ~2% of the original two-matmul implementation at every batch size. Full details, including the three-way results table, are in `RESULTS.md` at the repository root and in `benchmarks/README.md` section 39. As a result, this project is scoped to small-to-moderate batch sizes (1–32), which is also the batch-1-realistic regime for on-device / mobile inference.
